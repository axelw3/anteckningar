...
* $(F,\ +)$ är en kommutativ (abelsk) grupp med $0$ som identitet,
* $(F\setminus\{0\},\ \cdot)$ är en kommutativ grupp, samt
* $\cdot$ är *distributiv* m.a.p. $+$, det vill säga $a\cdot(b+c)=a\cdot b+a\cdot c$ för alla $a,b,c\in F$.

Exempel: $(\mathbb R,\ +,\ \cdot)$

* $\left(F,\ +\right)$ kallas för den *additiva gruppen* i F.
* $\left(F\setminus\{0\},\ \cdot\right)$ kallas för den *multiplikativa gruppen* i $F$.
...
På samma sätt kan är $R\lbrack x\rbrack$, där $R$ är en ring, mängden av alla polynom med koefficienter $a_i\in R$.
...
> **Sats 23.2:**
> 
> Låt $F$ vara en ändlig kropp med karakteristik $p$. Då är den edditiva gruppen $(F,\ +)\approx C_p\times\cdots\times C_p=\left(C_p\right)^r$ för något heltal $r\geq 1$. Speciellt är $|F|=p^r$.
...
Betecknas $\mathbb Z_3\lbrack x\rbrack/(x^2+x+2)$ och har element $\{\lbrack 0\rbrack,\ \lbrack 1\rbrack,\ \lbrack 2\rbrack,\ \lbrack x\rbrack,\ \lbrack x+1\rbrack,\ \lbrack x+2\rbrack,\ \lbrack 2x\rbrack,\ \lbrack 2x+1\rbrack,\ \lbrack 2x+2\rbrack\}$.
...
> **Sats:**
> 
> Låt $p$ vara ett primtal och $k(x)$ ett irreducibelt polynom i $\mathbb Z_p\lbrack x\rbrack$ med $\deg k=r\geq 1$.
> 
