...
## M3: Föreläsning 9
### Demand paging
* Hämta pages endast när de behövs.
* Valid/invalid-bit: v (memory resident) eller i (ej i minnet).
* Page fault: Åtkomst till en sida märkt "ogiltig" resulterar i en page fault.
    * Effektiv åtkomsttid (EAT): $(1-p)\cdot T_\text{mem\textunderscore access}+p\cdot T_\text{page\textunderscore fault}$, där $p$ är andelen page faults.
    * Hantering ("trap"): Operativsystemet ansvarar.
        1. Kontrollera att förfrågan är giltig (inom korrekt adressintervall).
        2. "Page in": Läs från disk till minne.
        3. Upprepa instruktionen som avbröts.

### Sidersättning
> **Free-frame list:** Länkad lista över fria frames.

* Om fri frame saknas, välj en *"offer-frame"* att ersätta.
    * Om denna är "dirty", skriv först data till disk!
    * Val av frame görs av en *sidersättnings-algoritm*.

#### Algoritmer
Jämföra algoritmer: Räkna antal sidfel över en sekvens av sidnummer.

Lokal ersättning: Vid ersättning kan *endast processens egna frames* väljas.

* FIFO: Ersätt sida som funnits i minne längst.
* Optimal: Ersätt den sida som inte kommer att användas på längst tid.
* Least-recently used (LRU): Ersätt den sida som minst nyligen använts.
    * Approximativ LRU: Varje sida har en *referens*-bit (initialt värde: 0). När en sida används ändras biten till 1.
        * "Klock-algoritm": Cirkulär kö av sidor. Välj den första med `ref=0`. För varje påträffad `ref=1`: ändra denna till `0`.
        * Liknar FIFO, med undantag för referens-biten.
* Least-frequently used: Ersätt den sida som har använts lägst antal gånger.

### Kernelminne i Linux
* "Buddy-systemet": Dela successivt ett minnesblock i två delar ("buddies"), så länge storleken är tillräcklig givet förfrågan.
* SLAB-allokator: Undviker fragmentering genom att *återanvända* tidigare allokerat minne vid nya allokeringar av samma typ/storlek.
    * För varje kernel-datastruktur finns en *slab-cache*.
    * En *slab* kan vara *tom*, *partiell* eller *full*. T.ex. kan en 12 kB-slab rymma 4x 3 kB-objekt. I första hand väljs en partiell slab -- annars används en tom slab. I sista hand kan en ny slab allokeras.
* Aktiv/inaktiv-lista: Page replacement i Linux *liknar den LRU-approximerande "klock-algoritmen"* och bygger på två listor ("active" & "inactive").

## M4: Föreläsning 10
### I/O-subsystemet
* Kernelns I/O-subsystem ansvarar för I/O-hantering.
* Typer av I/O-enheter:
    * Teckenenheter, t.ex. tangentbord
    * Nätverkenheter
        * *Sockets* är ett gränssnitt för nätverkskommunikation
    * Blockenheter: Överför data i *block* (för HDD: *sektorer*, för NVM: *pages*).

### I/O-hantering, komponenter

* Hårdvara: portar, bussar
    * I/O-port: Anslutningspunkt för enheter.
        * Har ofta "status", "control", "data-in" och "data-out"-register.
        * CPU ger instruktioner på två sätt:
            * Särskilda I/O-instruktioner för att överföra data
            * Minnes-översatt I/O.
    * Buss: Ledningar och protokoll för dataöverföring.
        * PCIe-buss: Ansluter processorns minnessubsystem till snabba I/O-enheter.
        * Expansionsbuss: Ansluter relativt långsamma enheter.
    * Styrenheter (*controller*): Mikroprocessorer som hanterar en port, buss eller enhet. Kör någon mjukvara och interagerar med drivrutinen.
* Kernel: använder drivrutinsmoduler

### Värd-enhet-interaktion
* Polling: Processorn kommunicerar med styrenheten genom att läsa/skriva register.
* Interrupt: Låter I/O-enheten "informera" processorn när något tillstånd ändras.
    * Vid interrupt hoppar processorn till en *interrupt-hanterare* (på en bestämd adress i minnet).
* DMA (direkt minnesåtkomst): Dataöverföring direkt mellan I/O-enhet och minne.
    * I allmänhet:
        1. Drivrutinen instrueras att överföra data till någon adress.
        2. Drivrutinen instruerar styrenheten.
        3. Styrenheten påbörjar DMA-överföring.
        4. DMA-styrenhet utför överföringen.
        5. DMA skickar en *interrupt* vid slutförande.

### I/O-gränssnitt (mjukvara)
* I/O-systemanrop: Funktioner som används generellt. Tack vare drivrutiner fungerar detta trots hårdvaruskillnader.
    * Möjliggör skydd mot otillåtna/skadliga I/O-operationer.
    * Synkront (*blocking*): `write()`, `close()`
    * Asynkront (*non-blocking*): `poll()`, `select()`
        * Blockerar inte (utförs i bakgrunden), men behöver istället ständigt ex.vis kontrollera om data finns tillgängligt.
        * I/O-subsystemet kan *signalera* när en åtgärd slutförs.

### Kernel
* Schemaläggning: Avgör lämplig ordning för utförande av I/O-åtgärder.
    * Ordningen kan t.ex. ha stor betydelse för läs/skriv-hastighet på hårddiskar.
    * För asynkron I/O: För att hålla koll på (potentiellt sett) en stor mängd I/O-förfrågningar används en kö.
        * Varje rad i *device-status-tabellen* (en per I/O-enhet) har en sådan kö.
* Buffring: Motverkar problem med hastighetsskillnader och avvikelser i överföringsstorlek.
* Cachning: Den snabbare enheten har en kopia av data.
* Felhantering: Försök igen och (vid många fel) inaktivera enheten.

### Enheter
* Hårddiskar (HDD): Skivor > Spår > Sektorer (=minsta överföringsstorlek)
* NVM: Styrenhet + datachip.
    * Snabbare överföring, men dyrare samt har kortare livstid.
    * Hela sidan måste rensas före skrivning. Detta sliter på enheten.

#### HDD-schemaläggning
* First come first serve (FCFS)
* Shortest seek time first (SSTF): Välj förfrågan som kräver minst söktid från nuvarande huvudposition.
    * En sorts SJF-schemaläggning -- kan leda till "starvation".
* SCAN: Förflyttning sker i huvudets riktning, till diskens slut nås -- vänder då. Köade förfrågningar hanteras en efter en allt eftersom de förekommer längs rörelsen.
    * Look: Vändning sker direkt efter sista förfrågan längs ursprungliga riktningen nås.
* C-SCAN: Efter vändning förflyttas huvudet till diskens början utan att läsa något längs vägen -- och fortsätter sedan därifrån i normal riktning till den tidigare/initiala positionen.
    * C-look: Kombination av C-SCAN & LOOK.

**Prestandamått:** Tidsåtgång ("access time") och bandbredd.

Minimera söktid: SSTF ger ofta bra resultat.

* SCAN/C-SCAN är mer *rättvisa* -- undviker "starvation".

#### NVM/NAND
* Datarensning görs i *block* (= flera sidor åt gången).
* Flash-styrenheten har en FTL (*flash translation layer*).
    * Tillåter omedelbar användning av tomma sidor.
    * Vid skrivning:
        1. Om tom ("giltig") sida finns: kombinera gammal + ny data och makera den gamla sidan som "ogiltig"
        2. Annars, om det finns ett block som kan rensas, vänta på detta före skrivning.
        3. I sista hand kan vi sammanfoga block som innehåller blandning av giltiga och ogiltiga sidor -- garbage collection.

##### Garbage collection
* Utförs av styrenheten.
* Giltiga sidor sammanfogas till ett block.
* Ogiltiga sidor sammanfogas till ett annan block -- hela blocket kan sedan rensas.
* Krävs ett "staging-utrymme" -- en mängd sidor åsidosatta för detta.

**Write amplification:** *På grund av garbage collection och space management kan en skrivoperation ta betydligt längre tid än en läsoperation.*

### RAID
* *Paritetsbitar* används för att upptäcka felaktiga bytes.
* *Felrättningskoder* (ECC) kan upptäcka och reparera fel.
    * Diskar använder sektorvis ECC, medan flashminnen använder sidvis ECC.

#### Data striping
* Bit-level striping: Dela upp alla bitar över flera diskar.
* Block-level striping: Håll samman hela block.

#### RAID-nivåer
* RAID0: Striping på blocknivå. Ingen redundans!
* RAID1: Fullständig kopia.
* RAID2/3: Ovanliga.
    * RAID2: Striping på bitnivå + paritet
    * RAID3: Striping på bytenivå + ECC
* RAID4: Striping på blocknivå + ECC (på separat disk).
* RAID5: Striping på blocknivå + ECC uppdelad *på samtliga diskar*.
* RAID6: Som RAID5, men med 2 paritetsblock istället för 1.
